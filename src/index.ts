...
export async function check(emailOrOptions: string | ValidatorOptions): Promise<OutputFormat> {
  ...
  if (options.validateMx) {
    ...
    if (options.validateSMTP) {
      return checkSMTP(options.sender, email, mx.exchange, options.helloName)
    }
  }
